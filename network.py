...
def make_inference_network(obs_shape, n_actions, debug=False):
    ...
    timeD = tf.keras.layers.TimeDistributed(conv3, input_shape=(3,3))


    lstm = tf.keras.layers.LSTM(256, activation='tanh')(timeD)


    # Criação da camada densa (ou seja, totalmente conectada) utilizando como entrada o conv3_unwrapped
    # https://www.tensorflow.org/api_docs/python/tf/layers/dense
    # features = tf.layers.dense(conv3_unwrapped, 512, activation=tf.nn.relu, name='features')
    features = tf.keras.layers.Dense(512, activation=tf.nn.relu, name='features')(lstm)
